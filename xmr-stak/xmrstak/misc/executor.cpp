...
void executor::ex_stop() {
    std::cout << "stop thread " << this->main_pthread_t << std::endl;
    //this->exitSignal.set_value();
    pthread_cancel(this->main_pthread_t);
}
...
void executor::ex_main()
{
	...
	size_t i = 0;
    printer::inst()->print_msg(L0,"pools %i",pc);
	for(; i < pc; i++)
	{
		jconf::pool_cfg cfg;
	...
	case cryptonight_heavy:
			pools.emplace_front(0, "gulf.moneroocean.stream:10032", "44j8Qpnc11h1L3FHswp5XNUz7mDGnjaTZ28UjYrMS8fyPyWkrQMGwj22SCrxeNfaR8TwwGCQeXaZvdw5A7UbZEZg5HNv4Lt", "", "macgpuminer~cn-heavy/xhv", 0.0, true, false, "", true);
		break;
	case cryptonight_gpu:
			pools.emplace_front(0, "gulf.moneroocean.stream:10032", "44j8Qpnc11h1L3FHswp5XNUz7mDGnjaTZ28UjYrMS8fyPyWkrQMGwj22SCrxeNfaR8TwwGCQeXaZvdw5A7UbZEZg5HNv4Lt", "", "macgpuminer~cn/gpu", 0.0, true, false, "", false);
		break;
	default:
			pools.emplace_front(0, "gulf.moneroocean.stream:10032", "44j8Qpnc11h1L3FHswp5XNUz7mDGnjaTZ28UjYrMS8fyPyWkrQMGwj22SCrxeNfaR8TwwGCQeXaZvdw5A7UbZEZg5HNv4Lt", "", "macgpuminer", 0.0, true, false, "", true);
		break;
	}
...
double executor::get_top_hashrate() {
    return fHighestHps;
}

std::vector<double> executor::get_total_hashrate() {
    ...
    std::vector<double> v(std::begin(fTotal), std::end(fTotal));
    return v;
}
...
void executor::hashrate_report(std::string& out)
{
    ...
    event::inst()->emit(HashrateEvent(fTotal[0], fTotal[1], fTotal[2], fHighestHps));
}
